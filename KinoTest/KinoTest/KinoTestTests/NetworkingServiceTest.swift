...
final class NetworkingServiceTest: XCTestCase {
    ...
    func testNetworkingServiceInternalServerError() {
        let config = URLSessionConfiguration.ephemeral
        config.protocolClasses = [MockURLProtocol.self]
        let session = URLSession(configuration: config)
        ...
        let networkingService = NetworkingService(session: session)
        let expectation = self.expectation(description: "Completion handler invoked on internal server error")
        
        networkingService.fetchData(urlString: "https://example.com/fail") { result in
            switch result {
            case .success(_):
                XCTFail("Expected internal server error, got success")
            case .failure(let error):
                XCTAssertEqual(error, APIError.serverResponseError)
                expectation.fulfill()
            }
        }
    ...
    func testFetchDataSuccess() {
        ...
        let networkingService = NetworkingService(session: session)
        let expectation = self.expectation(description: "Completion handler is expected to be invoked with success")
        
        networkingService.fetchData(urlString: "https://example.com/success") { result in
            switch result {
            case .success(let data):
                // S'assurer que les données reçues sont celles attendues
                XCTAssertNotNil(data, "Data should not be nil")
                let dataString = String(data: data, encoding: .utf8)
                XCTAssertEqual(dataString, "{\"key\": \"value\"}", "Unexpected data received")
                expectation.fulfill()
            case .failure(let error):
